...
class sword:
   def __init__(self, dmg):
       self.dmg = dmg
class wooden(sword):
    def __init__(self, dmg, hp, heal):
        super().__init__(dmg)
        self.hp = int(hp)
        self.heal = int(heal)
    def __str__(self):
        return f"{self.dmg}, {self.hp}, {self.heal}"
class zombie(sword):
   def __init__(self, dmg, ehp, eatk, edefe):
       super().__init__(dmg)
       self.ehp = int(ehp)
       self.eatk = int(eatk)
       self.edefe = int(edefe)
   def __str__(self):
       return f"{self.dmg}, {self.ehp}, {self.eatk}, {self.edefe}"
class enderman(zombie):
   def __init__(self, dmg, ehp, eatk, edefe):
       super().__init__(dmg, ehp, eatk, edefe)
   def __str__(self):
       return f"{self.dmg}, {self.ehp}, {self.eatk}, {self.edefe}"
